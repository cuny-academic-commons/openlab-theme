...
<div id="bbpress-forums">
	...
	<?php if ( is_user_logged_in() && current_user_can( 'edit_topic', bbp_get_topic_id() ) ) : ?>
		...
		<div id="split-topic-<?php bbp_topic_id(); ?>" class="bbp-topic-split">
			...
			<form id="split_topic" class="form form-panel" name="split_topic" method="post" action="<?php the_permalink(); ?>">

				<div class="bbp-form panel panel-default">

					<?php // translators: topic title ?>
					<div class="panel-heading"><?php echo esc_html( sprintf( __( 'Split topic "%s"', 'bbpress' ), bbp_get_topic_title() ) ); ?></div>
					...
					<div class="panel-body">
						...
						<fieldset class="bbp-form">
							...
							<div>
								...
								<?php // translators: forum title ?>
								<label for="bbp_topic_split_option_reply"><?php echo esc_html( sprintf( __( 'New topic in <strong>%s</strong> titled:', 'bbpress' ), bbp_get_forum_title( bbp_get_topic_forum_id( bbp_get_topic_id() ) ) ) ); ?></label>
								<?php // translators: topic title ?>
								<input type="text" id="bbp_topic_split_destination_title" value="<?php echo esc_html( sprintf( __( 'Split: %s', 'bbpress' ), bbp_get_topic_title() ) ); ?>" tabindex="<?php bbp_tab_index(); ?>" size="35" name="bbp_topic_split_destination_title" />
							</div>
